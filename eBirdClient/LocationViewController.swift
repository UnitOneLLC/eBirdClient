...
class SelectPointAnnotation : NSObject, MKAnnotation {
...
class LocationViewController: UIViewController, MKMapViewDelegate {
    ...
    var singlePin: Bool = false
    var singleAnno: MKAnnotation?
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if (targets!.count == 1 && targets![0].count == 1) {
            ...
            let annotation = SelectPointAnnotation(coordinate: targetPoint, title: target.caption, subtitle: target.subcaption)
            ...
            for group in targets! {
                if group.count > 0 {
                    ...
                    let annotation = SelectPointAnnotation(coordinate: group[0].coordinate, title: title, subtitle: subcaption)
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        if singlePin && singleAnno != nil {
            mapView.selectAnnotation(singleAnno!, animated: true)
        }
    }
    ...
    func mapView(mapView: MKMapView,
        viewForAnnotation annotation: MKAnnotation) -> MKAnnotationView? {
            ...
            if pinView == nil {
                pinView = LocationViewAnnotation(annotation: annotation, reuseIdentifier: reuseId)
    ...
    func mapView(mapView: MKMapView, didSelectAnnotationView view: MKAnnotationView) {
        selected = nil
        if let wrapper = (view.annotation as? SelectPointAnnotation)!.data as? Wrapper {
